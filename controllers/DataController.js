  ...
  try {
    ...
    for(var i = 0 ; i < length ; i++){
      ...
      for(var j = 0 ; j < lengths ; j++){
        let color;
        let panjang;
        var count = (parseFloat(datastmp.canal_data[i].data[j].depth) + parseFloat(datastmp.canal_data[i].water_level) + parseFloat(datastmp.canal_data[i].tranducer) + parseFloat(datastmp.canal_data[i].bed_float) - parseFloat(datastmp.canal_data[i].depth_correction)).toFixed(3);
        if(count < pengukuran[0].tidakLulus) {
          color = "red";
        } else if (count >= pengukuran[0].toleransi.batasAwal && count < pengukuran[0].toleransi.batasAkhir) {
          color = "blue";
...
export const getData = async (req,res) => {
  try {
    const data = await Data.find({ "canal_data._id" : req.params.id }, { "canal_data.$": 1});
    res.json(data);
  }catch (error) {
    res.status(404).json({message: error.message});
  }
...
export const getDataChart = async (req,res) => {
  try {
    var datas = [];
    const pengukuran = await Pengukuran.find();
    const datatmp = await Data.find({ "canal_data._id" : req.params.id }, { "canal_data.$": 1});
    var length = datatmp[0].canal_data[0].data.length;
    for(var j = 0 ; j < length ; j++){
      let color;
      var count = (parseFloat(datatmp[0].canal_data[0].data[j].depth) + parseFloat(datatmp[0].canal_data[0].water_level) + parseFloat(datatmp[0].canal_data[0].tranducer) + parseFloat(datatmp[0].canal_data[0].bed_float) - parseFloat(datatmp[0].canal_data[0].depth_correction)).toFixed(3);
      if( count < pengukuran[0].tidakLulus ) {
        color = "red";
      } else if ( count >= pengukuran[0].toleransi.batasAwal && count < pengukuran[0].toleransi.batasAkhir) {
        color = "blue";
              ...
              "in": {
                "data": {
                  "$filter": {
                    "input": "$$this.data",
                    "as": "data",
                    "cond": { "$eq": [ "$$data._id", mongoose.Types.ObjectId(req.params.id)] }
                  }
                }
              }
            }
...
export const saveData = async (req,res) => {
  try{
    const inserteddata = await Data.updateOne({_id: req.params.id}, {$push: req.body});
...
export const saveDetailData = async (req,res) => {
  try{
    ...
    const inserteddetaildata = await Data.updateOne({"canal_data._id": req.params.id}, {$push: detailData});
...
export const updateDatas = async (req,res) => {
  try {
    const updateddatas = await Data.updateOne({_id:req.params.id}, {$set: req.body});
...
export const updateData = async (req,res) => {
  try {
    const updateddata = await Data.updateOne({"canal_data._id":req.params.id}, {$set: req.body});
...
export const updateDetailData = async (req,res) => {
  try {
    const updateddetaildata = await Data.updateOne({"canal_data.data._id":req.params.id}, {$set: req.body}, {arrayFilters: [{"data._id":req.params.id}]});
...
export const deleteAllData = async (req,res) => {
  try {
    const deletedalldata = await Data.updateOne({_id: req.params.id}, {$pull: {'canal_data': {}}});
...
export const deleteData = async (req,res) => {
  try {
    const deleteddata = await Data.updateOne({"canal_data._id":req.params.id}, {$pull: {'canal_data': {'_id': req.params.id}}});
...
export const deleteAllDetailData = async (req,res) => {
  try {
    const deletedalldetaildata = await Data.updateOne({"canal_data._id": req.params.id}, {$pull: {'canal_data.$.data': {}}});
...
export const deleteDetailData = async (req,res) => {
  try {
    const deleteddetaildata = await Data.updateOne({"canal_data.data._id": req.params.id}, {$pull: {"canal_data.$.data" : {"_id":req.params.id}}});
