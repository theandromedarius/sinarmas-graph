...
const DetailDataList = () => {
              ...
              {response.data[0].canal_data[0].data.map((data, index) => (
                  ...
                  <td className='has-text-centered'>{data.depth}</td>
              	  <td className='has-text-centered'>
              	    ...
              	    <button onClick={() => deleteDetailData(data._id)} className='button is-danger is-small'>Delete</button>
              	  </td>
              	</tr>
              ))}
            </tbody>
  ...
  const exportDetailDataInfo = async () => {
    ...
    let water_level;
    var order_no = ("000000000000" + String(response.data[0].canal_data[0].order_no)).slice(-12);
    var operation_no = ("0000" + String(response.data[0].canal_data[0].operation_no)).slice(-4);
    var measure_point = ("000000000000" + String(response.data[0].canal_data[0].measure_point)).slice(-12);
    var revision = ("000" + String(response.data[0].canal_data[0].revision)).slice(-3);
    var usv_code = (String(response.data[0].canal_data[0].usv_code) + "               ").slice(0, 15);
    var canal_upper_width = checkNum(response.data[0].canal_data[0].canal_upper_width);
    var canal_bottom_width = checkNum(response.data[0].canal_data[0].canal_bottom_width);
    for(var i = 0; i < length; i++){
      var final_depth = parseFloat(response.data[0].canal_data[0].data[i].depth) + parseFloat(response.data[0].canal_data[0].tranducer) + parseFloat(response.data[0].canal_data[0].bed_float) - parseFloat(response.data[0].canal_data[0].depth_correction);
      ...
      if(response.data[0].canal_data[0].water_level < 0) {
        water_level = parseFloat(response.data[0].canal_data[0].water_level).toFixed(2)
      } else {
        water_level = parseFloat(response.data[0].canal_data[0].water_level).toFixed(3)
      }
      var mdtxt = ("S" + ("00000" + String(sta)).slice(-4) + "-" + String(water_level) + "-" + String(final_depth.toFixed(3)) + "-" + String(canal_upper_width)+ "-" + String(canal_bottom_width) + "/" + String(response.data[0].canal_data[0].measure_date) + "                                        ").slice(0, 40);
    ...
    if(parseInt(revision) === 0) {
      ...
      link.download = `${response.data[0].canal_data[0].district.code}-${response.data[0].canal_data[0].canal_id}-${response.data[0].canal_data[0].qc_date}-${response.data[0].canal_data[0].usv_code}-R${parseInt(revision)}.txt`;
    }
    link.href = url;
    link.click();
  }
  ...
  const sendFileData = async() => {
    try {
      ...
      var usv_code = (String(response.data[0].canal_data[0].usv_code) + "               ").slice(0, 15);
      for(var i = 0; i < length; i++){
        var mdtxt = ("S" + ("00000" + String(parseInt(response.data[0].canal_data[0].data[i].length)-20)).slice(-4) + "-" + String(parseFloat(response.data[0].canal_data[0].water_level).toFixed(3)) + "-" + String(parseFloat(response.data[0].canal_data[0].data[i].depth).toFixed(3)) + "-" + String(parseFloat(response.data[0].canal_data[0].canal_upper_width).toFixed(3)) + String(parseFloat(response.data[0].canal_data[0].canal_bottom_width).toFixed(3)) + "/" + String(response.data[0].canal_data[0].measure_date) + "                                        ").slice(0, 40);
      ...
      var fileData = tableHead + tableSubHead + content
      await axios.post(`http://localhost:5001/send/${response.data[0].canal_data[0].district_code}`,{
        "content": fileData
      });
      navigate(0);
    } catch (error) {
      ...
      <div className="column is-fullwidth">
        ...
        <div className='is-flex is-justify-content-space-between'>
          <span>
            ...
            <Link to={`chartDetail/${id}`} className='button is-info mr-5'>Export to Graph</Link>
            <button onClick={() => exportDetailDataInfo()} className='button is-info mr-5'>Export to txt</button>
            <button onClick={() => sendFileData()} className='button is-info mr-5'>Send to FTP</button>
          </span>
          <span>
            <Link 
              to={`adddetail/${id}`} 
            ...
            <button onClick={() => deleteAllDetailData(id)} className='button is-danger'>Clear Order</button>
          </span>
        </div>
      </div>
    </div>
