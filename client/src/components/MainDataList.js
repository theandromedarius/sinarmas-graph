    ...
    for(var i = 0; i < length; i++){
      ...
      for(var j = 0; j < dataLength; j++){
        ...
        if(j == 0){
          ...
          panjang = response.data.canal_data[i].canal_length - (dataLength-1)*20;
          if(panjang < 10){
        ...
        var mdtxt = ("S" + ("00000" + String(num)).slice(-4) + "-" + String(parseFloat(response.data.canal_data[i].water_level).toFixed(3)) + "-" + String(parseFloat(response.data.canal_data[i].data[j].depth).toFixed(3)) + "-" + String(canal_upper_width) + "-" + String(canal_bottom_width) + "/" + String(response.data.canal_data[i].measure_date) + "                                        ").slice(0, 40);
            ...
            <tbody >
              {response.data.map((data, index) => (
              	<tr key={data._id}>
                    ...
                    <button onClick={() => exportDataInfo(data._id)} className='button is-warning is-small mr-1 mb-1'>Export</button>
              	    <button onClick={() => deleteDatas(data._id)} className='button is-danger is-small '>Delete</button>
              	  </td>
              	</tr>
              ))}
            </tbody>
    ...
    <div className="columns">
      <div className="column is-fullwidth">
        ...
        <div className='is-flex is-justify-content-space-between'>
          <span></span>
          <span>
            <Link to={`addMainData`} className='button is-success mr-5'>Add Order</Link>
            <button onClick={() => deleteAllDatas()} className='button is-danger'>Clear Order</button>
          </span>
        </div>
      </div>
    </div>
